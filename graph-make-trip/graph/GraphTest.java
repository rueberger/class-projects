...
public class GraphTest {
    ...
    private static final int[][][] ETHREE = {
    ...
    /** Vertices traversed for breadth-first search of EONE from 0. */
    ...
    /** Vertices traversed for breadth-first search of ETHREE from 0. */
    ...
    public void makeUndirected() {
        ...
        for (int i : vertices(EONE)) {
            for (int j : vertices(EONE)) {
                assertEquals(String.format("bad containment for %d -> %d",
                                           i, j),
                             containsUndirected(i, j, EONE),
                             _G.contains(_V.get(i), _V.get(j)));
                for (int L = 0; L < numEdges(EONE); L += 1) {
                    assertEquals(String.format("bad containment for %d-%d->%d",
                                               i, L, j),
                                 containsUndirected(i, j, L, EONE),
                                 _G.contains(_V.get(i), _V.get(j), L));
                }
            }
        }
    ...
    public void removeDirected() {
        fillGraph(new DG(), EONE);
        int nv1 = numVertices(EONE);

        _G.remove(_V.get(2));
        assertEquals("wrong vertex count after removal", nv1 - 1,
                     _G.vertexSize());
        assertEquals("wrong edge count after removal", numEdges(ETWO),
                     _G.edgeSize());
